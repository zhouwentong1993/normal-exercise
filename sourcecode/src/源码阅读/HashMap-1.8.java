 ...
 *
 * 当 HashMap 中需要存储非常多的元素时，需要预初始化一个大的容量，否则在增加的过程中，会不断地 reHash。耗费性能
 ...
 *
 * HashMap 是非线程安全的，当多个线程同时访问时，只要有一个修改了结构，剩余的都不会同步。修改结构的操作包括 put delte 等。
 ...
 *
 * 可以通过 Collections.synchronizedMap 来实现一个线程安全的 map，多态的好处！！！
 ...
 *
 * entrySet 是遵循 fail-fast 原则的，一旦多个线程访问，读取的同时修改了，就会抛出并发修改异常。
...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     *
     * 在 JDK 1.8 中，通过 TreeMap 来替代 LinkedList 作为解决 hash 冲突的方式。（当长度 >= 8 时）
     ...
     */
    // 由树退化成链表必须满足两个条件，冲突数小于 8 && 最多 6 次 && 在 resize 时调用。通过 split 方法触发
     ...
     */
    // 基于 Map.Entry 的构建 HashMap 元素的基础组件
