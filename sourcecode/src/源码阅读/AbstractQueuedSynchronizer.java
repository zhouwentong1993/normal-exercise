...
/**
 * 提供了一个基于先进先出等待队列的阻塞锁和相关同步器，该类被定位成适应多种同步需求的实用基础组件，该类通过单原子整形变量表示状态
 ...
 *
 * 子类需定义 protected 方法去改变状态，按照对象锁释放或者获取去定义状态。
 * Subclasses must define the protected methods that change this state, and which
 * define what that state means in terms of this object being acquired
 * or released.
 *
 * 鉴于此，类中的其他方法是证明所有的队列和阻塞工作。子类可以拥有其他状态字段，但是只有自动更新的 int 值（指系统内置的）
 * 才可以被 #getState、#setState、#comapreAndSetState 等方法操纵，这些方法就相当于在操作同步器
 ...
 * to synchronization.
 *
 * 子类应当被定义成实现这些同步属性的非公有的内部帮助类，该类没有任何关于同步接口的实现，
 * 反而它定义了一些 #acquireInterruptibly 的方法，这种方法可以被具体的锁对象或者相关联的同步器去更合适地通过公有方法去调用
 ...
 *
 * 该类支持独占模式（默认）和共享模式两种，当在独占模式下获取锁时，另外试图获取锁的线程会失败。
 * 共享模式下多线程获取锁可能会成功。该类不清楚在当共享模式获取成功时的机器层面上的差异，
 * 下一个等待线程（如果存在的话）也要确定他是否可以获取，线程在不同的模式下共享同一个先入先出队列，
 * 通常情况下，子类实现仅支持一种模式，但是两者都可以在 ReadWriteLock 中发挥作用，
 * 仅支持独占或仅共享模式的子类无需定义支持未使用模式的方法。
...
public abstract class AbstractQueuedSynchronizer
     ...
     *
     * 在每一个 node 里面的 status 字段用来跟踪这个线程是否应该被阻塞
     ...
     * </pre>
     *
     * 入 CLH 队列只需要原子性地对尾节点进行操作。同样的，出队列也只需要更新首节点，稍微多一点的工作就是决定下一个成功的节点是什么。
