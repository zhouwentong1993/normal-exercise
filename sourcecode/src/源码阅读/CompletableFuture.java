 ...
 *
 * CompletableFuture 实现了 Future 和 CompletionStatge 接口。
 * <p>In addition to these and related methods for directly
 * manipulating status and results, CompletableFuture implements
 * interface {@link CompletionStage} with the following policies: <ul>
 *
 * 同步方法会被完成 CompletableFuture 的线程或者另外的调用 completion 方法的线程调用？
 * TODO 不太理解。
 ...
 *
 * 所有的异步方法默认被放到 ForkJoinPool#commonPool 来执行。
 * 除非不支持并行级别二以上，就会采用 new thread 的方式去执行每个任务。
...
public class CompletableFuture<T> implements Future<T>, CompletionStage<T> {
     ...
     * Overview:
     *
     * 不同的有依赖的操作会被放到一个 linked stack 数据结构中，通过 result field 来判断任务是该进还是该出。
     ...
     *
     * 引入 AltResult 类来对运行结果进行包装，里面有一个异常字段，如果为空，则代表没有发生异常。
     ...
     *
     * 定义了 Completion 接口，底下有不同的输入实现类。比如 single-input、two-input 等。
     ...
     *
     * Completion 类是用于函数计算的。基本上类似，Java 做了适配器工作，就不用用户自己做了
     *
    ...
    // Supplier 这个函数式接口不接收参数，并返回一个返回值。
    @SuppressWarnings("serial")
    static final class AsyncSupply<T> extends ForkJoinTask<Void>
        ...
        public void run() {
            ...
            if ((d = dep) != null && (f = fn) != null) {
                ...
                if (d.result == null) {
                    try {
                        // 和 Future 实现非常类似，completeValue 是更改 CompletableFuture 状态的方法。
                        // f.get() 是实际的调用逻辑
