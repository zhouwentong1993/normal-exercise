 ...
 * 额外的， terminated 方法可以通过被覆写去执行任何额外的处理，当 Executor 完全结束了。
...
public class ThreadPoolExecutor extends AbstractExecutorService {
    /**
     * 线程池里面的控制状态，是一个用 workerCount 和 runState 包装的 AtomicInteger 变量。
     * The main pool control state, ctl, is an atomic integer packing
     * two conceptual fields
     *   workerCount, indicating the effective number of threads 说明正在活跃线程
     ...
     *
     * workerCount 是正在运行中的线程，有微小的概率不准，比如生产线程失败和线程退出失败等情况。
     ...
     *
     * 运行中，能够处理新任务和在队列里面的任务
     *   RUNNING:  Accept new tasks and process queued tasks
     *   终止，不能接受新任务，但是能处理队列里面的任务
     *   SHUTDOWN: Don't accept new tasks, but process queued tasks
     *   停止，什么也不能干
     *   STOP:     Don't accept new tasks, don't process queued tasks,
     *             and interrupt in-progress tasks
     *   收拾，相当于打扫战场，所有的线程都终止，没有工作线程了。
        ...
        implements Runnable
    {
        ...
        Worker(Runnable firstTask) {
            ...
            // 由指定的线程工厂生产的线程，如果我覆写了线程工厂，在 newThread 方法里加上自己的逻辑，应该能够执行。
        ...
        // 尝试获取锁，看看人家这命名，unused，专业。
        // 类似的还有 catch Exception 时，用 ignore 来代替，更完美的方案是再加点儿注释，让别人明白为什么忽略
     ...
     */
    // 将 ctl 设置成 targetState，不太明白为啥要这样做。
     ...
     */
    // 终止掉空闲的工作线程，在 tryTerminal 方法里面为什么要只终止一个
     ...
     *
     * It also returns false if the thread
            ...
            if (t != null) {
                ...
                try {
                    ...
                    if (rs < SHUTDOWN ||
                        ...
                        // Worker 类没有覆写 equals 和 hashCode 方法。
                        workers.add(w);
                        int s = workers.size();
                        // 不断地变大
    ...
    private void processWorkerExit(Worker w, boolean completedAbruptly) {
        ...
        int c = ctl.get();
        // 要么是 shutdown，要么是 run
        if (runStateLessThan(c, STOP)) {
            if (!completedAbruptly) {
                // 线程池中 corePool 最少得有一个
        ...
        try {
            while (task != null || (task = getTask()) != null) {
                ...
                try {
                    ...
                    task = null;
                    // 我发现我执行 100w 个线程，这个数不准，是否是因为 long 的原因
                    // 记得 JVM 对 long 和 double 这种 64 位的无法保证准确性，有高位。
                    w.completedTasks++;
                    w.unlock();
                }
            }
    ...
    public void execute(Runnable command) {
         ...
         *
         * 1. 如果当前线程数少于 corePoolSize，将会尝试创建一个新线程并将当前指令作为其首个任务
         ...
         *
         * 2. 如果任务能顺利进入队列，我们仍然需要去双重检验我们是否添加成功了该线程。
         *
         * 3. If we cannot queue task, then we try to add a new
         * thread.  If it fails, we know we are shut down or saturated
         * and so reject the task.
         *
         * 3. 如果我们不能将任务放入队列，那么我们将会创建一个新线程。如果创建失败，我们就会拒绝这个任务。
         */
        int c = ctl.get();
        /*
         * 1. 如果当前线程数少于 corePoolSize，将会尝试创建一个新线程并将当前指令作为其首个任务
